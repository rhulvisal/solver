...
class TurnstileAPIServer:
    ...
    def __init__(self, headless: bool, useragent: Optional[str], debug: bool, browser_type: str, thread: int, proxy_support: bool, context_max_uses: int = DEFAULT_CONTEXT_MAX_USES):
        ...
        if self.browser_type in ['chromium', 'chrome', 'msedge'] and not useragent:
    ...
    async def _create_context_and_page(self, browser, config: dict, proxy: Optional[str] = None):
        context_options = {"user_agent": config['useragent']}
    ...
    def _make_solve_handler(self, target_url: str):
        ...
        async def handler(route):
            ...
            # Serve a blank HTML page at the target URL so we skip all real network I/O
            # while keeping window.location correct for Turnstile's domain check
    ...
    async def _solve_turnstile(self, task_id: str, url: str, sitekey: str, action: Optional[str] = None, cdata: Optional[str] = None):
        ...
        try:
            ...
            # Single handler: serves blank HTML at target URL (zero real network I/O),
            # serves cached turnstile JS, allows cloudflare, blocks everything else
            route_handler = self._make_solve_handler(target_url)
            await page.route("**/*", route_handler)

            # 'commit' exits as soon as navigation is committed — blank page is instant
            await page.goto(target_url, wait_until='commit', timeout=10000)

            await self._load_captcha_overlay(page, sitekey, action or '', index)

            # Short window for invisible turnstile auto-solve (resolves in <300ms or not at all)
            token = await self._wait_for_token(page, timeout=400)

            if not token:
                # Click-based solve with up to 3 attempts.
                # First attempt: allow 3s for iframe to attach + 3s for CF iframe content to load.
                # Retries: iframe already present + already loaded, so use short windows.
                for attempt in range(3):
                    iframe_wait = 3000 if attempt == 0 else 400
                    token_wait = 3000 if attempt == 0 else 2000
                    await self._click_turnstile_via_cdp(page, index, wait_ms=iframe_wait)
